...
describe 'BrowseNasaService' do
  ...
  context 'Browse the overall Asteroid data-set' do
    ...
    it 'should return a hash' do
      expect(@browse_service.get_hash_result).to eq Hash
    end

    it 'should return a string' do
      expect(@browse_service.get_orbiting_body_result_a_float).to be_a_kind_of(String)
    end
    ...
    it 'should have a date and be structured' do
      expect(@browse_service.get_close_approach_date_structure).to match /\A([12]\d{3}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01]))\z/
    end

    it 'should have the key' do
      expect(@browse_service.get_hash_key_should_match).to include("size" => 20)
    end
